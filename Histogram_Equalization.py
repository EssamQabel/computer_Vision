...
def Hist_Eq(img,hmin,hmax):
    ...
    y = np.zeros((row, col))
    m = 255.0 / (hmax-hmin)
    print("The histogram slope: " + str(m))
    for i in range(0,row):
        for j in range(0,col):
            print("Pixel (" + str(i) + ", " + str(j) + ") its value is: " + str(img[i, j]))
            if img[i,j] >= hmin and img[i,j] <= hmax:
                y[i, j] = math.floor(m * (img[i, j] - hmin))
                print("Its is equalized to " + str(y[i, j]))
            elif img[i, j] > hmax:
                y[i, j] = 255
            else:
                y[i, j] = 0
...
hist = Hist(img)
x = np.arange(0, 256)
plt.bar(x, hist, color='b', width=0.7, align='center')
plt.show()
